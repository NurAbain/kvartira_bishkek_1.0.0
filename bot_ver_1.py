...
def parser_hash_hamming(user, link, dist):
    ...
    with sq.connect('base_image.db') as con:
        cur = con.cursor()
        cur.execute(f"SELECT add_link FROM base where add_link =?", (link,))
    ...
    if cur.fetchone() is None:
        ...
        cur.execute(
            f'INSERT INTO base (add_link,images_hash) VALUES("{link}", "{finder_hash}")')
        con.commit()

        with sq.connect('base_image.db') as con:
            cur = con.cursor()
            base_image_hash = cur.execute("""SELECT add_link, images_hash FROM base""").fetchall()
        ...
        with sq.connect('base_image.db') as con:
            cur = con.cursor()
        image_hash = cur.execute(f"SELECT images_hash FROM base WHERE add_link = '{link}'").fetchone()
